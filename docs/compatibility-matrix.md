...
| **Custom Submenu Virtual Injection** (`koreader_adapter.lua`) | User-created submenus do not correspond to stock KOReader module registrations and would otherwise be rejected by `MenuSorter`. | KOReader lacks a dynamic runtime registry for user-defined menu categories. | Intercepts `mergeAndSort` to inject `KOMenu:custom_submenus` entries into `menu_table` prior to sorting. | Upstream KOReader provides an extensible menu provider API allowing plugins to register custom submenu containers dynamically. |
...
| **Clean Restart API Fallback** (`koreader_adapter.lua`) | Immediate in-session live refresh may fail or leave stale layout caches. | Older KOReader versions do not expose `UIManager:askForRestart`. | Checks `KoreaderAdapter.canRestart()`: uses native `UIManager:askForRestart` when available, falling back to a broadcast `ConfirmBox`. | When the minimum supported KOReader version includes `UIManager:askForRestart`. |
...
- [ ] Verify that the installed KOReader version includes a nil check in `frontend/ui/menusorter.lua`:
  ...
  ```
- [ ] Remove `KoreaderAdapter.installMenuSorterGuards()` in `main.lua` and `reorderingmenus_koreader_adapter.lua`.
...
- [ ] Remove `UICompat.installSortWidgetSubmenuTap` and `UICompat.releaseSortWidgetSubmenuTap` in `reorderingmenus_ui_compat.lua` and `reorderingmenus_ui_screens.lua`.
...
- [ ] Verify that `MenuSorter` copies `text_func` alongside `.text` during item ordering:
  ...
  ```
- [ ] Remove `UIScreens:sanitizeLiveMenuTree` in `reorderingmenus_ui_screens.lua`.
