...
5. **Unsupported Tab-Nesting Migration (legacy presets)**: Older presets may carry a top-level tab (e.g. Bookshelf) inside an ordinary submenu (`parent_override[tab] = More tools`, tab listed in that submenu's sequence, bar omission). Tabs cannot render nested (nil-text placeholder, crash when the host menu opens). On preset apply / dense import / external import, such placements are deterministically migrated: the tab stays in the bar with its children/callbacks intact, the nested sequence entry is stripped, and `tab_order` is filtered to live tabs. Resolve-time fallback (`Materializer.effectiveParent`) and validator repair (`removeNestedTabs`) keep even unsanitized worlds render-safe. The preset file itself is never rewritten, so the original bytes stay recoverable. Vanished ordinary containers (upstream-removed homes) are NOT migrated at ingest — they stay dormant in canonical intent and reapply when the home returns; unhide migrates a stale home to a valid one. Existing valid layouts, new presets, custom submenus, and independent view settings are unaffected.
