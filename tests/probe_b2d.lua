--[[--
probe_b2d.lua — B2 trace with correct manager name.
--]]
...
-- staged (what saveOrder would commit) + canonical after sync
local sec = Manager:stagedView(VIEW)
for id, po in pairs(sec.parent_override or {}) do
    print(string.format("STAGED po %s: provider=%s parent=%s anchor=%s",
        tostring(id), tostring(po.provider), tostring(po.parent),
        tostring(po.anchor)))
end
print("B2 pre-save file=" ..
    tostring(require("libs/libkoreader-lfs").attributes(ORDER_FILE, "mode")))
Manager:saveOrder(VIEW)
print("B2 post-save file=" ..
    tostring(require("libs/libkoreader-lfs").attributes(ORDER_FILE, "mode")))
