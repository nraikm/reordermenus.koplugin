--[[--
probe_b2g.lua — after beta launch, dump ALL staged parent_override records
to see who claims ghost_row's tools membership.
--]]
...
print("--- staged section after beta-era sync:")
local sec = Manager:stagedView(VIEW)
for id, po in pairs(sec.parent_override or {}) do
    print(string.format("  po[%s]: provider=%s parent=%s anchor=%s",
        tostring(id), tostring(po.provider), tostring(po.parent),
        tostring(po.anchor)))
end
for menu_id, oo in pairs(sec.order_override or {}) do
    print(string.format("  oo[%s]: n=%d", tostring(menu_id), #oo))
end
