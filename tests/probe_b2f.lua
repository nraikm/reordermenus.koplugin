--[[--
probe_b2f.lua — where does ghost_row's TOOLS membership come from under the
beta era? Dump the staged section's parent_override and the projection.
--]]
...
-- After beta-era sync (before save): what does canonical hold for ghost_row?
local sec = IntentStore.view(VIEW)
print("CANON po ghost_row:")
local po = sec.parent_override.ghost_row
if po then
    print(string.format("  provider=%s parent=%s anchor=%s",
        tostring(po.provider), tostring(po.parent), tostring(po.anchor)))
else
    print("  none")
end

-- What does the projection say?
print("projection parent=" ..
